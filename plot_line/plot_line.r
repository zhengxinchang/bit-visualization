...
#CONFIG:MODULENAME=Plot_Line
#CONFIG:APPNAME=Plot_Line
#CONFIG:DESC=This app draw a line plot for a given dataset.
#CONFIG:CLASS=/xtdisk/baoym_group/zhengxcngdc/softwares/miniconda3/envs/R36/bin/Rscript  /xtdisk/baoym_group/zhengxcngdc/projects/biocloud/units/plot_dot/plot_line.r
...
#CONFIG:HELP=J	8
#CONFIG:HELP=---------------------------------------------
#CONFIG:OUTFINENAME=output_plotline.png
...
option_list = list(
  ...
  make_option(c("--color"), type="character", default="black", help="line color"),
  make_option(c("--linewidth"), type="character", default="1", help="line width"), 
  make_option(c("--linetype"), type="character", default="solid", help="line type") #type=select:solid*,dashed,dotted,dotdash,longdash,twodash
)
...
objplot<- ggplot(dat) + 
  theme_classic() + 
  ggplot2::geom_line(
    aes_string(x=names(dat)[1],y=names(dat)[2]),
    linetype=myconvert(opt$linetype),
    size=myconvert(opt$linewidth),
    color=myconvert(opt$color),
    group=1
  ) 
...
ggsave(objplot,device = "png",filename =paste0(prefix,"/","output_plotline.png"),width = myconvert(opt$width),height = myconvert(opt$height),units = "in",dpi=600)

ggsave(objplot,device = "pdf",filename = paste0(prefix,"/","output_plotline.pdf"),width = as.numeric(opt$width),height = as.numeric(opt$height))

ggsave(objplot,device = "svg",filename = paste0(prefix,"/","output_plotline.svg"),width = as.numeric(opt$width),height = as.numeric(opt$height))
