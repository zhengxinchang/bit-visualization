...
if (!bit.check.is.null(opt$suggestiveline)) {
    paramlist[['suggestiveline']] <-  bit.convert.str2object(opt$suggestiveline)
}else{
    paramlist[['suggestiveline']] <-FALSE
    
}
if (!bit.check.is.null(opt$genomewideline)) {
    paramlist[['genomewideline']] <-  bit.convert.str2object(opt$genomewideline)
}else{
...
if ((!bit.check.is.null(opt$colora)) &&
    ...
    paramlist[['col']] <- c(bit.convert.str2object(opt$colora),bit.convert.str2object(opt$colorb))
    ...
    if(!bit.check.is.null(opt$palette)){
        ...
        this_palette<- bit.palette_list_generation_function[[opt$palette]](length(unique(datsnp$CHR)))
...
# ggsave(objplot,device = "png",filename =paste0(fullplotname,".png"),width =  bit.convert.str2object(opt),height =  bit.convert.str2object(opt),units = "in",dpi=600)
#
# ggsave(objplot,device = "pdf",filename = paste0(fullplotname,".pdf"),width =  bit.convert.str2object(opt),height = bit.convert.str2object(opt),units = "in")
#
# ggsave(objplot,device = "svg",filename = paste0(fullplotname,".svg"),width =  bit.convert.str2object(opt),height =  bit.convert.str2object(opt),units = "in")
#
#
#
#
...
# a <-bit.palette_list_generation_function[[ bit.convert.str2object('aaas')]] (12)
#
#
#
# manhattan(gwasResults, main = "Manhattan Plot",  col =a,
#           suggestiveline = T, genomewideline = F, chrlabs = c(1:20, "P", "Q"),
#           annotateTop =FALSE,highlight = snpsOfInterest)
#

# pramaters for plot
# ylim = c(-10, 10), cex = 0.9,
# cex.axis = 0.9,
#
#
# annotateTop = True
...
# cex annotatePval 有效
#
# write.table(as.data.frame(c(1:20, "P", "Q")),file = "./plot_manhattan/plot_manhattan.example.chrname.txt",col.names = F,quote = F,row.names = F)
#
# write.table(snpsOfInterest,file = "./plot_manhattan/plot_manhattan.example.highlight.txt",col.names = F,quote = F,row.names = F)
#
# write.table(gwasResults,file = "./plot_manhattan/plot_manhattan.example.snp.txt",quote = F,row.names = F)
